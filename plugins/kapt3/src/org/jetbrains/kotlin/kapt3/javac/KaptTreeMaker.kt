...
class KaptTreeMaker(context: Context) : TreeMaker(context) {
    ...
    fun FqName(internalOrFqName: String): JCTree.JCExpression {
        val path = internalOrFqName.replace('/', '.').let { handleSpecialFqNames(it) }.split('.')
    ...
    private fun handleSpecialFqNames(fqName: String): String {
        // Hard-coded in ImplementationBodyCodegen, KOTLIN_MARKER_INTERFACES
        if (fqName == "kotlin.jvm.internal.markers.KMutableMap\$Entry") {
