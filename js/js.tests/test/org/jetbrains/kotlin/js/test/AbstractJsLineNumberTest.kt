...
open class AbstractJsLineNumberTest : KotlinTestWithEnvironment() {
    fun doTest(filePath: String) {
        ...
        val linesMatcher = LINES_PATTERN.matcher(sourceCode)
        TestCase.assertTrue("'// LINE: ' comment was not found in source file", linesMatcher.find())

        val expectedLines = linesMatcher.group(1).trim()
        ...
        TestCase.assertEquals(programOutput.toString(), expectedLines, actualLines)
    }
    ...
    companion object {
        private val LINES_PATTERN = Regex("^ *// *LINES: *(.*)$", RegexOption.MULTILINE).toPattern()
    }
}